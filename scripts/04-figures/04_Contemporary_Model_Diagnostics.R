...
rhats_contemp <- map2(
  ...
  ~ .x$rhat %>% 
    ...
    labs(title = parse(text = paste(
        "bold('Gelman-Rubin '*hat(R)*' Diagnostic for", 
        .y, "')"))) +
...
# map(rhats_contemp, ~ print(.x))

# Save the generated plot object as a .jpeg file
map2(
  .x = contemp_rhat_files,
  .y = rhats_contemp,
  ~ ggsave(
    filename = .x,
    plot = .y,
...
# map(neff_contemp, ~ print(.x))

# Save the generated plot object as a .jpeg file
map2(
  .x = contemp_neff_files,
  .y = neff_contemp,
  ~ ggsave(
    filename = .x,
    plot = .y,
...
# map(nuts_contemp, ~ print(.x))

# Save the generated plot object as a .jpeg file
map2(
  .x = contemp_nuts_files,
  .y = nuts_contemp,
  ~ ggsave(
    filename = .x,
    plot = .y,
...
trace_highlight_contemp <- map2(
  ...
  ~ mcmc_trace_highlight(
    ...
    labs(
      y = "", 
      x = "Iteration",
      title = str_c("MCMC Highlighted Trace Plots for ", .y),
...
# map(trace_contemp, ~ print(.x))

# Save the generated plot objects as a .jpeg file
map2(
  .x = contemp_trace_files,
  .y = trace_contemp,
  ~ ggsave(
    filename = .x,
    plot = .y,
...
# map(rank_contemp, ~ print(.x))

# Save the generated plot objects as a .jpeg file
map2(
  .x = contemp_rank_files,
  .y = rank_contemp,
  ~ ggsave(
    filename = .x,
    plot = .y,
...
diag_plots_contemp <- map(
  ...
  ~ (
    rhats_contemp[[.x]] + 
      ...
      nuts_contemp[[.x]] + 
      ggtitle("No U-Turn Sampler Energy Diagnostic")) / trace_contemp[[.x]] + 
...
# Save the generated plot objects as a .jpeg file
map2(
  .x = contemp_diagplot_files,
  .y = diag_plots_contemp,
  ~ ggsave(
    filename = .x,
    plot = .y,
    device = "jpeg",
