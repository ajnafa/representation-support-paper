...
contemp_forms <- formula_builder(lhs = base_form,  rhs = terms_forms)

# Prepare the brms formula for each model
contemp_forms_bf <- map(
  .x = contemp_forms,
...
priors_contemp_sgc_cond_linvar_M0 <-
...
priors_contemp_sgc_cond_linvar_M1_M3 <- priors_contemp_sgc_cond_linvar_M0 +
  prior(normal(0, 0.5), class = "b", coef = "female_wi:libdem_wi") + 
  prior(normal(0, 0.5), class = "b", coef = "female_wi:libdem_be")

# Specify Priors for the model M4
priors_contemp_sgc_cond_linvar_M4 <- priors_contemp_sgc_cond_linvar_M0 +
  prior(normal(0, 0.5), class = "b", coef = "female_wi:pctfemleg_wi") + 
  prior(normal(0, 0.5), class = "b", coef = "female_wi:pctfemleg_be")

# Specify Priors for the model M5
priors_contemp_sgc_cond_linvar_M5 <- priors_contemp_sgc_cond_linvar_M1_M3 +
  prior(normal(0, 0.5), class = "b", coef = "female_wi:pctfemleg_wi") + 
  prior(normal(0, 0.5), class = "b", coef = "female_wi:pctfemleg_be")

# Specify Priors for the model M10
priors_contemp_sgc_cond_linvar_M10 <- priors_contemp_sgc_cond_linvar_M4 +
  prior(normal(0, 0.5), class = "b", coef = "female_wi:gdp_pcap_wi") + 
  prior(normal(0, 0.5), class = "b", coef = "female_wi:gdp_pcap_be")

# Add all the priors to a list object
contemp_priors <- list(
  priors_contemp_sgc_cond_linvar_M0,
  priors_contemp_sgc_cond_linvar_M1_M3,
  priors_contemp_sgc_cond_linvar_M0,
  priors_contemp_sgc_cond_linvar_M1_M3,
  priors_contemp_sgc_cond_linvar_M4,
  priors_contemp_sgc_cond_linvar_M5,
  priors_contemp_sgc_cond_linvar_M0,
  priors_contemp_sgc_cond_linvar_M0,
  priors_contemp_sgc_cond_linvar_M4,
  priors_contemp_sgc_cond_linvar_M5,
  priors_contemp_sgc_cond_linvar_M10
)
...
sgc_cond_contemp_linvar_fits <- map(
  .x = seq_along(contemp_forms_bf),
  .f = ~ brm(
    formula = contemp_forms_bf[[.x]],
    family = brmsfamily("bernoulli", link = "logit"),
    prior = contemp_priors[[.x]],
...
sgc_cond_contemp_linvar_fits <- map(
  .x = sgc_cond_contemp_linvar_fits,
  .f = ~ add_criterion(
    .x,
    criterion = "loo",
    cores = 1,
